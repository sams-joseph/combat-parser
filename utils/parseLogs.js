...
function getSpell(index, event, line) {
  ...
  if (line.indexOf('Your') < 0 || line.indexOf('You') < 0) {
    if (data.critical) {
      ...
      data.spellName = line.slice(index, line.indexOf(event) - 1);
    }
  } else if (data.critical) {
    data.spellName = line.slice(line.indexOf('Your') + 5, line.indexOf('critically') - 1);
  } else data.spellName = line.slice(line.indexOf('Your') + 5, line.indexOf(event) - 1);

  if (event === 'heals') {
    data.amount = Number(line.slice(line.indexOf('for') + 4, line.indexOf('points') - 1));
  } else if (event === 'hits') {
    data.amount = Number(line.slice(line.indexOf('for') + 4, line.indexOf('damage') - 1));
  }
...
function parseAll(line, playerName) {
  ...
  if (line.includes('heals')) data.spell = getSpell(line.indexOf("'s") + 3, 'heals', line);
  else if (line.includes('hits')) data.spell = getSpell(line.indexOf("'s") + 3, 'hits', line);
